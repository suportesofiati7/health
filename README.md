# Sofiati Premium Bilingual Website Concept System
...
This repository replaces the previous 50-industry portfolio with 50 premium Sofiati website concepts. Every concept is Portuguese by default, English-switchable, mobile-first, ethical, and based on the supplied Sofiati Brand Identity folder.
...
python3 scripts/build_selector_pages.py
python3 scripts/translate_pages.py
python3 scripts/audit_static_site.py
python3 scripts/capture_homepage_screenshots.py
```

`generate_concepts.py` rebuilds the English-source concept pages and concept CSS from `data/concepts.json`. `translate_pages.py` rewrites static HTML to Portuguese and regenerates `js/translations.js` so the header utility switcher can restore English. `audit_static_site.py` checks completeness, local links, ethical copy, service coverage, disclaimers, screenshots and tracked video files. `capture_homepage_screenshots.py` writes desktop and mobile homepage screenshots to `final/homepage-screenshots/`.

## Rules

- Portuguese is the default reader language. Keep the English switcher available for developer/client review.
...
/scripts/         Build, translation, audit, screenshot and git helper scripts
```
...
Each page loads `/partials/header.html`, `/partials/footer.html`, `/partials/mobile-menu.html` and `/partials/concept-switcher.html` through `/js/partials.js`. Use a local HTTP server so partial loading works consistently; pages include fallback header/footer markup if a fetch fails. The header partial includes the minimal PT/EN utility switcher.
...
```bash
scripts/git_update.sh "Sofiati website update - 2026-06-24"
scripts/git_update_allow_empty.sh "Sofiati website update - 2026-06-24"
```

The first helper commits real changes and pushes the current branch. The second allows an empty commit when a deployment marker is useful.
