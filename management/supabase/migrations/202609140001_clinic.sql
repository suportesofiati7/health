...
create table public.memberships (
  ...
  role text not null check(role in ('owner','professional','reception','readonly')),
...
create table public.entries (
  ...
  kind text not null check(kind in ('assessment','anamnesis','plan','consultation','evolution','note','consent','alert')),
...
create table public.appointments (
  ...
  status text not null default 'scheduled' check(status in
    ('scheduled','confirmed','waiting','in_consultation','completed','cancelled','no_show','rescheduled')),
...
create table public.documents (
  ...
  name text not null check(length(name)<=200), category text not null default 'document', description text not null default '',
...
begin
  if TG_OP='UPDATE' then
    if old.status='final' then raise exception 'final_record_is_immutable'; end if;
    ...
    if new.amends_id is not null and not exists(select 1 from public.entries
      where id=new.amends_id and patient_id=new.patient_id and organization_id=new.organization_id and status='final')
    then raise exception 'invalid_amendment'; end if;
  end if;
  new.updated_at:=now();
  new.finalized_at:=case when new.status='final' then now() else null end;
  return new;
end $$;
...
create policy org_read on public.organizations for select to authenticated using(private.has_role(id,array['owner','professional','reception','readonly']));
create policy members_read on public.memberships for select to authenticated using(private.has_role(organization_id,array['owner','professional','reception','readonly']));
do $$ declare t text; begin
  foreach t in array array['patients','appointments','tasks','admin_notes','enquiries'] loop
    execute format('create policy staff_read on public.%I for select to authenticated using(private.has_role(organization_id,array[''owner'',''professional'',''reception'',''readonly'']))',t);
  end loop;
  foreach t in array array['patients','appointments','tasks'] loop
    execute format('create policy staff_insert on public.%I for insert to authenticated with check(private.has_role(organization_id,array[''owner'',''professional'',''reception'']) and created_by=auth.uid())',t);
    execute format('create policy staff_update on public.%I for update to authenticated using(private.has_role(organization_id,array[''owner'',''professional'',''reception''])) with check(private.has_role(organization_id,array[''owner'',''professional'',''reception'']))',t);
  end loop;
  foreach t in array array['entries','entry_versions','documents','document_links','communications'] loop
    execute format('create policy clinical_read on public.%I for select to authenticated using(private.has_role(organization_id,array[''owner'',''professional'']))',t);
  end loop;
end $$;
create policy note_insert on public.admin_notes for insert to authenticated with check(private.has_role(organization_id,array['owner','professional','reception']) and created_by=auth.uid());
create policy entries_insert on public.entries for insert to authenticated with check(private.has_role(organization_id,array['owner','professional']) and created_by=auth.uid());
create policy entries_update on public.entries for update to authenticated using(private.has_role(organization_id,array['owner','professional']) and created_by=auth.uid() and status='draft') with check(private.has_role(organization_id,array['owner','professional']) and created_by=auth.uid());
create policy links_insert on public.document_links for insert to authenticated with check(private.has_role(organization_id,array['owner','professional']) and created_by=auth.uid());
create policy enquiry_update on public.enquiries for update to authenticated using(private.has_role(organization_id,array['owner','professional','reception'])) with check(private.has_role(organization_id,array['owner','professional','reception']));
create policy audit_read on public.audit_events for select to authenticated using(private.has_role(organization_id,array['owner']));
...
begin
  ...
  if not found or not private.has_role(e.organization_id,array['owner','professional','reception']) then raise exception 'not_authorized'; end if;
  ...
  update public.enquiries set patient_id=p,status='converted',updated_at=now(),version=version+1 where id=e.id;
  return p;
end $$;
...
language plpgsql security definer set search_path='' as $$
begin
  if event not in ('patient_open','patient_export','document_download') or not private.has_role(org,array['owner','professional']) then raise exception 'not_authorized'; end if;
  if event='document_download' then
  ...
  insert into public.audit_events(organization_id,actor_id,action,entity_type,entity_id)
  values(org,auth.uid(),event,case when event='document_download' then 'documents' else 'patients' end,entity);
end $$;
...
begin
  ...
  select coalesce(sum(size_bytes),0) into used from public.documents where status<>'failed';
  ...
  if found then
    if d.status='ready' then return d; end if;
...
begin
  if not private.has_role('a783bd4c-f253-4a94-9365-75c6f1000001',array['owner']) then raise exception 'not_authorized'; end if;
  select jsonb_build_object('bytes',coalesce(sum(size_bytes),0),'files',count(*),'limit',800000000,
    'database_bytes',pg_database_size(current_database())) into result from public.documents where status<>'failed';
  return result;
end $$;
...
create policy private_document_read on storage.objects for select to authenticated using(
  bucket_id='patient-files' and exists(select 1 from public.documents d where d.path=storage.objects.name and d.status='ready'
    and private.has_role(d.organization_id,array['owner','professional']))
);
