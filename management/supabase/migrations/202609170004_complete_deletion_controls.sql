...
-- Keep permanent deletion server-authoritative and complete across every
-- patient-linked module. Audit events intentionally remain as the immutable
-- compliance record of what was deleted.
...
begin
  ...
  paid := exists (
    ...
    where patient_id = p.id and status = 'recebido'
  );
  ...
  -- Delete children before their patient/document parents. Tables with
  -- explicit ON DELETE CASCADE are still listed where they contain sensitive
  -- records, making the data-retention behavior clear and migration-safe.
  delete from public.patient_portal_sessions
    where account_id in (select id from public.patient_portal_accounts where patient_id = p.id);
  delete from public.patient_portal_accounts where patient_id = p.id;
  delete from public.patient_portal_shares where patient_id = p.id;
  ...
  delete from public.product_usages where clinical_procedure_id in
    (select id from public.clinical_procedures where patient_id = p.id);
  delete from public.clinical_procedures where patient_id = p.id;
  delete from public.treatment_plan_items where plan_id in
    (select id from public.treatment_plans where patient_id = p.id);
  ...
  delete from public.entry_versions where entry_id in
    (select id from public.entries where patient_id = p.id);
  ...
  delete from public.communication_messages where conversation_id in
    (select id from public.communication_conversations where patient_id = p.id);
