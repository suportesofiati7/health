...
-- Storage objects are owned by Supabase Storage. The files Edge Function
-- removes them through the Storage API; the relational RPC must only remove
-- database rows. Direct deletes from storage.objects can fail in production
-- because Storage metadata is protected by internal triggers.
