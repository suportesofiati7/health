...
function printReceipt(record, patient, payments, t) {
  ...
  :root{color:#1d3026;background:#f8f5ee}*{box-sizing:border-box}body{margin:0;padding:16mm;font:12px/1.45 Inter,Arial,sans-serif}.receipt{max-width:720px;min-height:250mm;margin:auto;background:#fffdf8;border:1px solid #e2d9cc;box-shadow:0 8px 30px #39251b12;padding:30px 38px;position:relative;overflow:hidden}.receipt:before,.receipt:after{content:"";position:absolute;width:180px;height:180px;border:1px solid #dba99b;border-radius:50%;opacity:.35}.receipt:before{right:-100px;top:-90px}.receipt:after{left:-120px;bottom:-100px}header{border-bottom:2px solid #3f5039;padding-bottom:16px;margin-bottom:20px}.logo{width:180px;height:auto;display:block;margin-bottom:7px}h2{text-align:center;letter-spacing:.08em;font:400 19px Georgia,serif;margin:15px 0 22px}.brand{font:400 20px Georgia,serif}.brand span{display:block;font:9px Inter,Arial,sans-serif;letter-spacing:.16em;color:#7c453e;margin-top:3px}.meta{color:#52634b;font-size:11px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px 24px;margin-bottom:20px}.label{display:block;color:#52634b;font-size:10px;margin-bottom:3px}.line{display:flex;justify-content:space-between;border-bottom:1px solid #e8e1d7;padding:8px 0}.summary{background:#f1f5ec;border-radius:5px;padding:11px 14px;margin-top:18px}.summary div{display:flex;justify-content:space-between;margin:4px 0}.balance{color:#7c453e;font-size:15px;font-weight:700}.signature{text-align:center;margin:24px auto 0;width:220px;border-top:1px solid #789078;padding-top:6px;font-size:11px}.footer{border-top:1px solid #dce2d7;margin-top:20px;padding-top:12px;text-align:center;font-size:10px;color:#52634b}button{padding:10px 16px;border:1px solid #dce2d7;background:white;color:#3f5039;cursor:pointer}@media print{body{padding:0;background:white}.receipt{box-shadow:none;border:0;max-width:none;min-height:0}button{display:none}@page{size:A4;margin:10mm}}</style></head><body><button id="print">${escapeHTML(t("Imprimir / salvar PDF", "Print / save PDF"))}</button><article class="receipt"><header><img class="logo" src="${LOGO}" alt="Franciele Sofiati"/><div class="brand">Franciele Sofiati<span>BIOMÉDICA · ESTÉTICA AVANÇADA</span></div><p class="meta">${escapeHTML(t("Recibo de pagamento · não é Nota Fiscal", "Payment receipt · not a fiscal invoice"))}</p></header><h2>${escapeHTML(t("RECIBO DE PAGAMENTO", "PAYMENT RECEIPT"))}</h2><div class="grid"><div><span class="label">${escapeHTML(t("Recebemos de", "Received from"))}</span><strong>${escapeHTML(patient?.preferred_name || patient?.full_name || "—")}</strong></div><div><span class="label">CPF</span>${escapeHTML(patient?.cpf || "—")}</div><div><span class="label">${escapeHTML(t("Procedimento", "Treatment"))}</span>${escapeHTML(record.procedure_name || "—")}</div><div><span class="label">${escapeHTML(t("Recibo nº", "Receipt no."))}</span>${escapeHTML(record.receipt_number || record.id.slice(0, 8).toUpperCase())}</div></div><p class="meta"><strong>${escapeHTML(paymentWording)}</strong> · ${escapeHTML(t("Emitido em", "Issued on"))} ${escapeHTML(date(new Date().toISOString()))}</p><h3>${escapeHTML(t("Pagamentos recebidos", "Payments received"))}</h3>${rows || `<p class="meta">${escapeHTML(t("Nenhum pagamento registrado", "No payments recorded"))}</p>`}<div class="summary"><div><span>${escapeHTML(t("Valor total do tratamento", "Total treatment amount"))}</span><strong>${escapeHTML(money(total / 100))}</strong></div><div><span>${escapeHTML(t("Valor recebido", "Amount received"))}</span><strong>${escapeHTML(money(received / 100))}</strong></div><div class="balance"><span>${escapeHTML(t("Saldo pendente", "Remaining balance"))}</span><strong>${escapeHTML(money(balance / 100))}</strong></div></div><p>${escapeHTML(t("Recebemos a importância acima referente ao procedimento descrito.", "We received the amount above for the treatment described."))}</p><div class="signature">Franciele Sofiati<br><strong>Biomédica Esteta · CRBM 6277 PR</strong><br>Londrina, Paraná</div><p class="footer">${escapeHTML(t("AUTOCUIDADO TAMBÉM É INVESTIMENTO.", "SELF-CARE IS AN INVESTMENT TOO."))}</p></article></body></html>`);
