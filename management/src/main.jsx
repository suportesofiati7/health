...
function ProcedureCatalog({ notify }) {
  ...
  const uploadProcedureImage = async (file) => {
    if (!file) return;
    if (!file.type?.startsWith("image/") || file.size > 8 * 1024 * 1024) {
      notify(t("Escolha uma imagem de até 8 MB.", "Choose an image up to 8 MB."));
      return;
    }
    setBusy(true);
    try {
      ...
      const extension = file.type === "image/jpeg" ? "jpg" : file.type.split("/")[1].replace("svg+xml", "svg");
      const path = `${ORG}/${user.id}/procedure-${crypto.randomUUID()}.${extension}`;
      await checked(db.storage.from("profile-photos").upload(path, file, { upsert: false, contentType: file.type }));
      set("image_url", `storage://${path}`);
      notify(t("Imagem enviada. Salve o procedimento para aplicar.", "Image uploaded. Save the procedure to apply it."));
    } catch {
      notify(t("Não foi possível enviar a imagem. Verifique sua conexão e tente novamente.", "Could not upload the image. Check your connection and try again."));
    } finally { setBusy(false); }
  };
  ...
  return <section className="detail-section">
    ...
    {!!editing && <form className="form-grid" onSubmit={saveProcedure}>
      ...
      <Field title={t("Imagem do serviço (URL)", "Service image (URL)")} value={form.image_url?.startsWith("storage://") ? "" : form.image_url} onChange={(v) => set("image_url", v)} wide />
      <label className="field wide"><span>{t("Enviar imagem do serviço", "Upload service image")}</span><input type="file" accept="image/*" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; uploadProcedureImage(file); }} /><small>{t("Imagem própria · até 8 MB. Também é possível informar um endereço HTTPS acima.", "Your image · up to 8 MB. You can also enter an HTTPS address above.")}</small></label>
      <div className="procedure-image-editor-preview wide"><ProcedureImage procedure={form} /><small>{t("Prévia 16:9 · se a imagem enviada ou o endereço falhar, será exibida a imagem local sugerida para o serviço.", "16:9 preview · if an uploaded image or address fails, the local suggested service image will appear.")}</small></div>
