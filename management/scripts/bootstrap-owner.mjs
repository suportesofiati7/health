...
try {
  ...
  if(existing&&existing.role!=='owner')throw Error('Existing account has a different role. Review the existing account before bootstrap.');
  ...
  if(!existing){const {error:insertError}=await db.from('memberships').insert({organization_id:ORG,user_id:data.user.id,email,name:'Franciele Sofiati',role:'owner',status:'invited',profession:'Biomedicina',council:'CRBM',registration:'6277',state:'PR'});if(insertError)throw Error('Could not create owner membership. No password was created.');}
