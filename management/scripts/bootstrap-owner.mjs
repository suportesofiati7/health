...
try {
  ...
  let developerLink='';
  if(!developer.data){const invite=await db.auth.admin.generateLink({type:'invite',email:developerEmail,options:{redirectTo:url}});if(invite.error)throw Error('Could not create the Ash Tra Auth account.');const {error:developerInsert}=await db.from('memberships').insert({organization_id:ORG,user_id:invite.data.user.id,email:developerEmail,name:'Ash Tra',role:'proprietario',status:'convidado',profession:'Software Developer'});if(developerInsert)throw Error('Could not create the Ash Tra membership.');developerLink=`${url}/#token_hash=${encodeURIComponent(invite.data.properties.hashed_token)}&type=invite`;}
