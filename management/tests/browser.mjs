...
const patient={id:'30000000-0000-4000-8000-000000000001',organization_id:org,full_name:'Ashlyn Ann Merrigan',preferred_name:'Ashlyn',birth_date:'1992-03-12',phone:'43999990000',email:'ashlyn.merrigan@example.invalid',cpf:null,rg:'',cns:null,address:{},guardian:{},emergency_contact:{},status:'ativo',avatar_path:`${org}/patient/ashlyn-qa.png`,created_at:created,version:1};
...
const fixtures={memberships:[member],patients:[patient],entries:[entry],entry_versions:[],documents:[],document_links:[],clinical_photos:[{id:'91000000-0000-4000-8000-000000000001',organization_id:org,patient_id:patient.id,path:`${org}/${patient.id}/ashlyn-qa.png`,category:'antes',area:'Face completa',description:'Imagem fictícia para validação visual.',created_at:created}],communications:[],admin_notes:[],public_intakes:[intake],enquiries:[intake],financial_records:[financeRecord],financial_payments:[financePayment],appointments:[{id:'50000000-0000-4000-8000-000000000001',organization_id:org,patient_id:patient.id,patients:patient,starts_at:`${day}T14:00:00-03:00`,ends_at:`${day}T15:00:00-03:00`,status:'confirmado',label:'Avaliação estética',version:1}],tasks:[{id:'60000000-0000-4000-8000-000000000001',organization_id:org,patient_id:patient.id,patients:patient,title:'Confirmar retorno fictício',due_at:`${day}T10:00:00-03:00`,status:'pendente',priority:'alta',version:1}],audit_events:[]};
...
async function contextFor(width,height,role='proprietario'){
  ...
  await context.route('**/naypgbhwnlbyqqqfftgn.supabase.co/**',async route=>{
    ...
    if(path.includes('/storage/v1/object/'))return route.fulfill({status:200,contentType:'image/png',body:Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=','base64')});
...
try{
  for(const [width,height]of[[1440,1000],[390,844],[768,1024]]){
    ...
    for (const tab of ['Dados','Documentos e fotos','Fotografia clínica','Planos','Procedimentos','Saúde','Consentimentos','Privacidade / portal','Agenda e retornos','Administrativo','Histórico']) { await page.getByRole('button',{name:tab,exact:true}).last().click(); await noOverflow(page); }
    await page.getByRole('button',{name:'Fotografia clínica',exact:true}).click(); await page.locator('.clinical-photo-thumb img').waitFor({state:'attached'}); assert.equal(await page.locator('.clinical-photo-thumb img').evaluate((image) => image.naturalWidth > 0),true);
    ...
    await page.getByLabel('Valor recebido (R$)').fill('700');await page.getByRole('button',{name:'Registrar pagamento',exact:true}).click();assert(posts.some(p=>p.table==='financial_payments'&&p.body.amount_cents===70000));await page.getByRole('button',{name:'Nota Fiscal',exact:true}).click();await page.getByRole('heading',{name:'Nota Fiscal',exact:true}).waitFor();await page.getByLabel('Link oficial').fill('https://nf.example.invalid/FS-2026-TESTE');await page.getByRole('button',{name:'Salvar Nota Fiscal',exact:true}).click();assert(posts.some(p=>p.table==='financial_records'&&p.body.nota_fiscal_url==='https://nf.example.invalid/FS-2026-TESTE'));await page.getByRole('button',{name:'Corrigir / Reembolsar',exact:true}).click();await page.getByLabel('Motivo').fill('Estorno fictício');await page.getByRole('button',{name:'Confirmar estorno',exact:true}).click();assert(posts.some(p=>p.table==='financial_payments'&&p.body.status==='estornado'&&p.body.refund_reason==='Estorno fictício'));await page.getByRole('button',{name:'Usar tema escuro',exact:true}).click();assert.equal(await page.evaluate(()=>document.documentElement.dataset.theme),'dark');await page.getByRole('button',{name:'Usar tema claro',exact:true}).click();
    await navigation(page,'Relatórios');await page.getByRole('heading',{name:'Relatórios',exact:true}).waitFor();await page.locator('.report-filters select').first().selectOption(patient.id);await noOverflow(page);const reportPopup=page.waitForEvent('popup');await page.getByRole('button',{name:'Gerar PDF',exact:true}).click();const reportPage=await reportPopup;await reportPage.waitForLoadState('domcontentloaded');await reportPage.getByRole('heading',{name:'Prontuário completo do paciente',exact:true}).waitFor();await reportPage.screenshot({path:`test-results/report-${width}.png`,fullPage:true});const headerMetrics=await reportPage.evaluate(()=>{const header=document.querySelector('.document-header'),content=document.querySelector('.document-content');const h=header?.getBoundingClientRect(),c=content?.getBoundingClientRect();return {headerBottom:h?.bottom,contentTop:c?.top,overlap:!!(h&&c&&h.bottom>c.top)}});assert.equal(headerMetrics.overlap,false,`document header overlaps content: ${JSON.stringify(headerMetrics)}`);await reportPage.getByRole('button',{name:'Imprimir / salvar PDF',exact:true}).waitFor();await reportPage.close();
