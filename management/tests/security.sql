...
case right(id::text,1) when '1' then 'owner' when '2' then 'professional' when '3' then 'reception' else 'readonly' end,
case right(id::text,1) when '5' then 'inactive' else 'active' end from auth.users;
...
('80000000-0000-4000-8000-000000000001','a783bd4c-f253-4a94-9365-75c6f1000001','30000000-0000-4000-8000-000000000001','fictional.pdf','clinic1/fictional.pdf','application/pdf',100,repeat('a',64),'ready','10000000-0000-4000-8000-000000000001'),
('80000000-0000-4000-8000-000000000002','a783bd4c-f253-4a94-9365-75c6f1000002','30000000-0000-4000-8000-000000000002','other.pdf','clinic2/other.pdf','application/pdf',100,repeat('b',64),'ready','10000000-0000-4000-8000-000000000001');
...
insert into public.entries(id,organization_id,patient_id,kind,content) values('40000000-0000-4000-8000-000000000001','a783bd4c-f253-4a94-9365-75c6f1000001','30000000-0000-4000-8000-000000000001','consultation','Fictional clinical content');
...
update public.entries set status='final' where id='40000000-0000-4000-8000-000000000001';
...
select pg_temp.denied($q$insert into public.entries(organization_id,patient_id,kind) values('a783bd4c-f253-4a94-9365-75c6f1000001','30000000-0000-4000-8000-000000000002','note')$q$,'cross-organization patient reference');
select pg_temp.denied($q$update public.patients set created_by='10000000-0000-4000-8000-000000000002'$q$,'author spoofing');
insert into public.entries(organization_id,patient_id,kind,amends_id,content) values('a783bd4c-f253-4a94-9365-75c6f1000001','30000000-0000-4000-8000-000000000001','note','40000000-0000-4000-8000-000000000001','Fictional amendment');
...
select set_config('request.jwt.claims',jsonb_build_object('sub','10000000-0000-4000-8000-000000000003','session_id','20000000-0000-4000-8000-000000000003','exp',extract(epoch from now()+interval '1 hour'),'role','owner')::text,true);
...
select pg_temp.denied($q$update public.enquiries set status='new' where id='70000000-0000-4000-8000-000000000001'$q$,'converted enquiry cannot be silently unlinked');
select pg_temp.assert((select count(*)=0 from public.audit_events),'reception denied audit log');
select pg_temp.denied($q$insert into public.entries(organization_id,patient_id,kind) values('a783bd4c-f253-4a94-9365-75c6f1000001','30000000-0000-4000-8000-000000000001','note')$q$,'reception clinical insert');
select pg_temp.denied($q$update public.memberships set role='owner'$q$,'self promotion');
