# 45 Silhouette Design DNA

Concept name: Silhouette

Visual idea: clinical skincare drops translated into a Sofiati visual world, using clinical drop capsules as the governing asset language.
...
Navigation idea: drop-commerce header on desktop and regimen menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: full-height consultation portrait badge, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-silhouette.webp`.

Botanical/gold idea: botanical section separator appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom clinical drop capsules SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: gold shimmer controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: step-style consultation form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: botanical map footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `clinical drop capsules`, portrait treatment `full-height consultation portrait badge`, motion `gold shimmer`, form `step-style consultation form` and footer `botanical map footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: silhouette image-led minimalism.
...
- Visual storytelling style: Silhouette should feel like silhouette image-led minimalism, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `46` and `03` during duplicate-layout audits.
