...
Visual idea: graceful portrait-led care translated into a Sofiati visual world, using service shelf labels as the governing asset language.
...
Navigation idea: portrait-led service navigation on desktop and flowing portrait menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: soft oval botanical frame, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-grace.webp`.

Botanical/gold idea: soft leaf watermark appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom service shelf labels SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.
...
How this concept differs from the other 49: It is identified by `service shelf labels`, portrait treatment `soft oval botanical frame`, motion `menu bloom`, form `sidebar form` and footer `concierge contact footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: graceful treatment diary.
...
- Visual storytelling style: Grace should feel like graceful treatment diary, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `17` and `24` during duplicate-layout audits.
