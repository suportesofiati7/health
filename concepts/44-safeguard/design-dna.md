# 44 Safeguard Design DNA

Concept name: Safeguard

Visual idea: cream skincare commerce translated into a Sofiati visual world, using cream commerce labels as the governing asset language.
...
Navigation idea: cream commerce header on desktop and cream category menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: business-card portrait with signature line, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-safeguard.webp`.

Botanical/gold idea: monogram wreath appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom cream commerce labels SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: icon tilt controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: gold line form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: clinical column footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `cream commerce labels`, portrait treatment `business-card portrait with signature line`, motion `icon tilt`, form `gold line form` and footer `clinical column footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: safeguard careful decision system.
...
- Visual storytelling style: Safeguard should feel like safeguard careful decision system, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `45` and `02` during duplicate-layout audits.
