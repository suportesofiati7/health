# 37 Vellum Design DNA

Concept name: Vellum

Visual idea: treatment pass system translated into a Sofiati visual world, using treatment pass slots as the governing asset language.
...
Navigation idea: pass-booking header on desktop and pass drawer menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: clinical credential portrait card, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-vellum.webp`.

Botanical/gold idea: animated leaf path appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom treatment pass slots SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: divider trace controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: floating editorial form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: apothecary fine-print footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `treatment pass slots`, portrait treatment `clinical credential portrait card`, motion `divider trace`, form `floating editorial form` and footer `apothecary fine-print footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: vellum paper-and-proof system.
...
- Visual storytelling style: Vellum should feel like vellum paper-and-proof system, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `38` and `45` during duplicate-layout audits.
