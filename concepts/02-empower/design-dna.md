# 02 Empower Design DNA

Concept name: Empower

Visual idea: decision-led consultation studio translated into a Sofiati visual world, using business-card seal system as the governing asset language.
...
Navigation idea: sticky appointment decision bar on desktop and authority contact-sheet menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: round monogram-backed portrait stamp, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-empower.webp`.

Botanical/gold idea: sage line-art branch appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom business-card seal system SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: leaf orbit controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: sage panel form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: large monogram ledger using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `business-card seal system`, portrait treatment `round monogram-backed portrait stamp`, motion `leaf orbit`, form `sage panel form` and footer `large monogram ledger`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: decision-led consultation studio.
...
- Visual storytelling style: Empower should feel like decision-led consultation studio, not a recolor of another concept.
...
- Mobile layout style: single-column image ledger; mobile content must stay readable, image-led, and action-oriented.
...
- Border/radius system: arched portrait frames; avoid one-note rounded-card repetition.
...
- Resemblance risks: compare closely against concepts `03` and `10` during duplicate-layout audits.
