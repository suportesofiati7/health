# 23 Ritual Design DNA

Concept name: Ritual

Visual idea: medical skin landing translated into a Sofiati visual world, using medical skin disclosure set as the governing asset language.
...
Navigation idea: treatment proof header on desktop and condition menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: sage duotone editorial profile tile, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-ritual.webp`.

Botanical/gold idea: bronze corner frame appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom medical skin disclosure set SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: portrait lift controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: botanical framed form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: signature seal footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `medical skin disclosure set`, portrait treatment `sage duotone editorial profile tile`, motion `portrait lift`, form `botanical framed form` and footer `signature seal footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: ritual care journal.
...
- Visual storytelling style: Ritual should feel like ritual care journal, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `24` and `31` during duplicate-layout audits.
