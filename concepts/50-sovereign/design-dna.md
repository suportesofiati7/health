# 50 Sovereign Design DNA

Concept name: Sovereign

Visual idea: minimalist ingredient lab translated into a Sofiati visual world, using ingredient lab minimal grid as the governing asset language.
...
Navigation idea: ingredient lab header on desktop and ingredient drawer menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: journal-side portrait notecard, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-sovereign.webp`.

Botanical/gold idea: footer botanical stamp appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom ingredient lab minimal grid SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: service badge pulse controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: portrait-led consultation form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: credential stamp footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `ingredient lab minimal grid`, portrait treatment `journal-side portrait notecard`, motion `service badge pulse`, form `portrait-led consultation form` and footer `credential stamp footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: sovereign editorial authority.
...
- Visual storytelling style: Sovereign should feel like sovereign editorial authority, not a recolor of another concept.
...
- Mobile layout style: stacked story cards; mobile content must stay readable, image-led, and action-oriented.
...
- Border/radius system: sharp editorial edges; avoid one-note rounded-card repetition.
...
- Resemblance risks: compare closely against concepts `01` and `08` during duplicate-layout audits.
