# 48 Signature Design DNA

Concept name: Signature

Visual idea: botanical serum story translated into a Sofiati visual world, using botanical serum line growth as the governing asset language.
...
Navigation idea: botanical story header on desktop and serum story menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: champagne line portrait seal, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-signature.webp`.

Botanical/gold idea: quote-mark branch appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom botanical serum line growth SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.

Animation idea: form focus glow controls strip dividers, icon hover behaviour, portrait movement and scroll CSS variables.

Form idea: business-card form with a botanical frame, focus glow, privacy acknowledgement, honeypot, loading, success and error states.

Footer idea: bronze strip footer using the white logo, monogram wreath, public contact routes and Londrina, PR only.

How this concept differs from the other 49: It is identified by `botanical serum line growth`, portrait treatment `champagne line portrait seal`, motion `form focus glow`, form `business-card form` and footer `bronze strip footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: signature brand dossier.
...
- Visual storytelling style: Signature should feel like signature brand dossier, not a recolor of another concept.
...
- Mobile layout style: compact route rail; mobile content must stay readable, image-led, and action-oriented.
...
- Border/radius system: fine-line panels; avoid one-note rounded-card repetition.
...
- Resemblance risks: compare closely against concepts `49` and `06` during duplicate-layout audits.
