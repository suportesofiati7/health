# 22 Precision Design DNA

Concept name: Precision

Visual idea: consultant authority translated into a Sofiati visual world, using signature consultant trace as the governing asset language.
...
Navigation idea: doctor-signature header on desktop and consultant menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: round monogram-backed portrait stamp, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-precision.webp`.

Botanical/gold idea: sage line-art branch appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom signature consultant trace SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.
...
How this concept differs from the other 49: It is identified by `signature consultant trace`, portrait treatment `round monogram-backed portrait stamp`, motion `leaf orbit`, form `sage panel form` and footer `large monogram ledger`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: precision technical atlas.
...
- Visual storytelling style: Precision should feel like precision technical atlas, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `23` and `30` during duplicate-layout audits.
