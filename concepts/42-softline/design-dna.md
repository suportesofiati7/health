# 42 Softline Design DNA

Concept name: Softline

Visual idea: apothecary minimalism translated into a Sofiati visual world, using apothecary text stamps as the governing asset language.
...
Navigation idea: apothecary header on desktop and text-only apothecary menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: round monogram-backed portrait stamp, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-softline.webp`.

Botanical/gold idea: sage line-art branch appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom apothecary text stamps SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.
...
How this concept differs from the other 49: It is identified by `apothecary text stamps`, portrait treatment `round monogram-backed portrait stamp`, motion `leaf orbit`, form `sage panel form` and footer `large monogram ledger`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: softline gentle pathway.
...
- Visual storytelling style: Softline should feel like softline gentle pathway, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `43` and `50` during duplicate-layout audits.
