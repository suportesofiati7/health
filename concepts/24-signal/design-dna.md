# 24 Signal Design DNA

Concept name: Signal

Visual idea: fine clinic calm translated into a Sofiati visual world, using fine-list micro glyphs as the governing asset language.
...
Navigation idea: ultra-minimal header on desktop and quiet list menu on mobile, with botanical background assets and a small portrait cue in the menu.
...
Doctor portrait idea: business-card portrait with signature line, generated from `brand identity/Dr Fran.png` and saved as `assets/portrait/franciele-portrait-signal.webp`.

Botanical/gold idea: monogram wreath appears in the hero, section strip, mobile menu, form and footer.

Icon idea: A custom fine-list micro glyphs SVG icon pack covers consultation, laser, skin, care, evaluation, safety, aftercare, results, journal, mission, values, contact, technology, credentials, WhatsApp and back to top.
...
How this concept differs from the other 49: It is identified by `fine-list micro glyphs`, portrait treatment `business-card portrait with signature line`, motion `icon tilt`, form `gold line form` and footer `clinical column footer`. The HTML, CSS and JS include unique markers for this concept.

## Required Planning Expansion

- Concept mood: signal-led decision guide.
...
- Visual storytelling style: Signal should feel like signal-led decision guide, not a recolor of another concept.
...
- Resemblance risks: compare closely against concepts `25` and `32` during duplicate-layout audits.
